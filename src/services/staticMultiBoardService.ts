...
- Staff POD: PM + FE + BE + Design; timebox 12 weeks`;
    ...
    if (role.includes('senior product manager') || role === 'spm' || role === 'pm' || role.endsWith('product manager')) {
...
- Annotate releases; run A/B only if stable`;
    ...
    if (role.includes('engineer') || role.includes('platform') || role.includes('tech lead')) {
